...
int main(void) {
  // file
  FILE *out_file = fopen("./logs/test_gemm_acim_v3_result.txt", "w");
  if (out_file == NULL) {
  ...
  for (std::array<int, 3> &workload : workloads) {
    ...
    // ========================================
    // warm up
    warm_up(gemm_acim_v3, A, B, C, M, N, K, in_scale, wt_scale, norm_input_bw, norm_weight_bw,
            out_input_bw, out_weight_bw, quant, warm_up_iter);

    // test
    run_test(gemm_acim_v3, A, B, C, M, N, K, in_scale, wt_scale, norm_input_bw, norm_weight_bw,

...
#define HGEMV_LOG_TAG "HGEMV"
#define HGEMV_LOG_FILE(x) (strrchr(x, '/') ? (strrchr(x, '/') + 1) : x)
#define HLOG(format, ...)                                                                          \
  do {                                                                                             \
    fprintf(stderr, "[%s %s %d:%ld %s:%d %s] " format "\n", HGEMV_LOG_TAG, curr_time(), get_pid(), \
            get_tid(), HGEMV_LOG_FILE(__FILE__), __LINE__, __FUNCTION__, ##__VA_ARGS__);           \
  } while (0)

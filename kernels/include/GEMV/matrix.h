...
public:
    Matrix(size_t row, size_t col, const std::string &name = "Matrix", float min = -1.0, float max = 1.0)
        ...
        HGEMV_CHECK_GT(m_row, 0);
        HGEMV_CHECK_GT(m_col, 0);

        m_elem_num = m_row * m_col;
        HGEMV_CHECK_GT(m_elem_num, 0);

        m_host_ptr = new half[m_elem_num];
        HGEMV_CHECK(m_host_ptr);
        HGEMV_CHECK_CUDART_ERROR(cudaMalloc((void **)&m_dev_ptr, m_elem_num * sizeof(half)));
        HGEMV_CHECK(m_dev_ptr);
        ...
        HGEMV_CHECK_CUDART_ERROR(cudaMemcpy(m_dev_ptr, m_host_ptr, m_elem_num * sizeof(half), cudaMemcpyHostToDevice));
    ...
    ~Matrix() {
        ...
        if (m_dev_ptr) {
            HGEMV_CHECK_CUDART_ERROR(cudaFree((void *)m_dev_ptr));
            m_dev_ptr = nullptr;
        }
    }
    ...
    void tearUp(Matrix *base) {
        HGEMV_CHECK(base);
        HGEMV_CHECK_EQ(m_row, base->getRow());
        HGEMV_CHECK_EQ(m_col, base->getCol());
    ...
    void moveToHost() {
        HGEMV_CHECK_CUDART_ERROR(cudaMemcpy(m_host_ptr, m_dev_ptr, m_elem_num * sizeof(half), cudaMemcpyDeviceToHost));
    }
    ...
    void memSetDevice() {
        HGEMV_CHECK_CUDART_ERROR(cudaMemset(m_dev_ptr, 0, m_elem_num * sizeof(half)));
    }
...
private:
    ...
    // the threshold of the random matrix will affect the difference of the hgemv results
    ...
    HGEMV_DISALLOW_COPY_AND_ASSIGN(Matrix);
};

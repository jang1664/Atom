...
public:
    explicit Tester(size_t N = 256, size_t K = 128, size_t warmup_iterations = 1, size_t profiling_iterations = 10,
        ...
        m_A = new Matrix(1, m_K, "Vector A");
        HGEMV_CHECK(m_A);
        m_B = new Matrix(m_K, m_N, "Matrix B");
        HGEMV_CHECK(m_B);
        m_C = new Matrix(1, m_N, "Vector C");
        HGEMV_CHECK(m_C);
        m_base = new Matrix(1, m_N, "Vector Base");
        HGEMV_CHECK(m_base);
...
private:
    ...
    Matrix *m_A = nullptr;     // row major, 1 * K
    Matrix *m_B = nullptr;     // col major, K * N
    Matrix *m_C = nullptr;     // row major, 1 * N
    Matrix *m_base = nullptr;  // row major, 1 * N, base result, init vector C before each hgemv

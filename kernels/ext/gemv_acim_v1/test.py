...
if __name__ == "__main__":
  ...
  for i in range(16):
    ...
    eval_output_data = torch.zeros_like(ref_output_data)

    for m in range(M):
      eval_output_data[m:m+1, :] = gemv_acim_v1.forward(input_data[m:m+1, :].to(torch.uint8).to(DEV), weight_data.to(torch.uint8).to(DEV), in_scale_data.to(DEV), weight_scale_data.to(DEV), 4, 4, False).reshape(ref_output_data[m:m+1, :].shape)
